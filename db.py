...
conn.execute('''CREATE TABLE issuedTo 
    (issuedfrom STRING, 
    productname STRING references inventory(productId), 
    battalionId INTEGER references battalions(battalionId), 
    districtId INTEGER references district(districtId),
    policestation INTEGER references policestation(psId),
    quantity INTEGER)''')
